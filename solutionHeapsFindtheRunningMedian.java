...
public class Solution {
	public static void main(String[] args) {
		...
		for(int i=0; i < n; i++){
			...
			// double med;
			// if ( (c % 2) != 0 ) {
			// 	med = (double)minHeap.items[c/2] ;
			// } else {
			// 	med = ((double)minHeap.items[ (c/2)-1 ] + (double)minHeap.items[ (c/2) ] )/2;
			// }
			//System.out.printf("%.1f %d \n", med, c/2 );
		}
		for(int j=0; j < minHeap.size; j++){
			//System.out.printf("%d %d %d %d / ", j, minHeap.leftChild(j), minHeap.rightChild(j), minHeap.parent(j) );
